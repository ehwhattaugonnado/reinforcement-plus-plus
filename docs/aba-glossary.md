...
1. **Citation attributed to the wrong preference-assessment method.** [Product Spec §7](product-spec.md#7-educational-references) currently cites "DeLeon & Iwata (1996), Evaluation of a multiple-stimulus presentation format for assessing reinforcer preferences" as an educational reference — but that paper describes and evaluates **MSWO**, which is explicitly **out of scope for v1** ([Product Spec §4](product-spec.md#4-mvp-scope)). The method v1 actually implements — four stimuli, six unique pairs, one pass through all pairings — is the **paired-stimulus (forced-choice) method**, whose original source is **Fisher, W. W., Piazza, C. C., Bowman, L. G., Hagopian, L. P., Owens, J. C., & Slevin, I. (1992). A comparison of two approaches for identifying reinforcers for persons with severe and profound disabilities. *Journal of Applied Behavior Analysis, 25*, 491–498.** *(Applied below.)*

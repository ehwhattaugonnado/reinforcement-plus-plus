...
**Relevance:** **This is the closest textbook anchor to [ADR 0003](adr/0003-eligibility-vs-experienced-consequences-invariant.md), the project's "eligibility vs. experienced consequences" invariant.** A labeled schedule (e.g., VR-3) describes the long-run intended average, not a promise about any specific delivery. CRF encodes this distinction with `criterion-met` (eligibility) and `stimulus-delivered` (the experienced consequence); ADR 0010's VR implementation instead judges each actual delivery against the round's running average because it has no discrete due instant.
